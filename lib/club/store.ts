 ...
 *
 * Upstash REST supports MULTI/EXEC batches but not WATCH, so optimistic
 * locking uses official compare-and-set primitives instead: SET NX for
 * create-if-absent, and createScript/EVALSHA Lua that only GET/SET/DEL the
 * exact stored JSON. Lua does not re-encode rooms with cjson.
 */
export const MAX_MEMBERS = 12;
export const CREATE_CODE_ATTEMPTS = 5;
const CAS_ATTEMPTS = 8;
const DEFAULT_KEY_PREFIX = "room:";
const RAW_GET_PREFIX = "W|";
...
/** Minimal Upstash surface used by the Redis store (and in-process CAS fakes). */
export interface ClubRedis {
...
function isSetNxOk(result: unknown): boolean {
  return result === "OK" || result === true;
}

function isCasSuccess(result: unknown): boolean {
...
const RAW_GET_SCRIPT = `
-- wedges:raw-get
local v = redis.call('GET', KEYS[1])
if not v then
  return false
end
return '${RAW_GET_PREFIX}' .. v
`;
...
function decodeRawGet(value: unknown): string | null {
  if (value === false || value == null) return null;
  if (typeof value !== "string" || !value.startsWith(RAW_GET_PREFIX)) return null;
  return value.slice(RAW_GET_PREFIX.length);
}

function redisScripts(redis: ClubRedis) {
  return {
    loadRaw: redis.createScript<unknown>(RAW_GET_SCRIPT),
    casSet: redis.createScript<unknown>(CAS_SET_SCRIPT),
    casDel: redis.createScript<unknown>(CAS_DEL_SCRIPT),
  };
}

function redisStore(redis: ClubRedis, keyPrefix: string): ClubStore {
  ...
  async function load(code: string): Promise<{ raw: string; room: Room } | { raw: null; room: null } | "unavailable"> {
    try {
      const raw = decodeRawGet(await scripts.loadRaw.exec([key(code)], []));
  ...
  async function casWrite<T>(
    code: string,
    apply: (room: Room) => StoreResult<T & { room: Room }>,
  ): Promise<StoreResult<T & { room: Room }>> {
    for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt++) {
      ...
      const result = apply(cloneRoom(loaded.room));
      if (!result.ok) return result;
      try {
        const swapped = await scripts.casSet.exec([key(code)], [loaded.raw, serializeRoom(result.room)]);
        if (isCasSuccess(swapped)) return result;
      } catch {
  ...
  return {
    async get(code) {
      const loaded = await load(code);
      if (loaded === "unavailable" || !loaded.room) return null;
      return cloneRoom(loaded.room);
    },
    async createIfAbsent(room) {
      try {
        const wrote = await redis.set(key(room.code), serializeRoom(room), { nx: true });
        if (isSetNxOk(wrote)) return { ok: true, room: cloneRoom(room) };
        return { ok: false, error: "conflict" };
      } catch {
        return { ok: false, error: "unavailable" };
      }
    ...
    async joinMember(input) {
      return casWrite(input.code, (room) => {
        const result = applyJoin(room, input);
        return result.ok ? { ok: true, member: structuredClone(result.member), room: result.room } : result;
      });
    },
    async appendSubmission(input) {
      return casWrite(input.code, (room) => {
        const result = applyAppend(room, input);
        return result.ok ? { ok: true, submission: structuredClone(result.submission), room: result.room } : result;
      });
    },
    async deleteIfOwner(code, ownerToken) {
      for (let attempt = 0; attempt < CAS_ATTEMPTS; attempt++) {
        ...
        try {
          ...
          if (isCasSuccess(swapped)) return { ok: true, deleted: true };
        } catch {
          return { ok: false, error: "unavailable" };
        }
      }
...
let testMutex: Promise<unknown> = Promise.resolve();
...
/**
 * Swap the process singleton for one test at a time so parallel files cannot
 * clobber each other's in-memory rooms.
 */
export async function acquireStoreForTests(next: ClubStore): Promise<() => void> {
  let release!: () => void;
...
/** Test-only: swap the process singleton. Prefer acquireStoreForTests. */
