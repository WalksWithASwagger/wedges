...
import { acquireStoreForTests, createClubStore, MAX_MEMBERS, type ClubStore } from "@/lib/club/store";
...
beforeEach(async () => {
  ...
  releaseStore = await acquireStoreForTests(store);
});
...
test("create route uses create-if-absent and reports bounded collisions", async () => {
  ...
  releaseStore?.();
  releaseStore = await acquireStoreForTests({
    ...store,
...
test("capacity race through the join route admits exactly one member", async () => {
  ...
  assert.equal(failedBody.error, "room_full");
  assert.equal(failedBody.message, "This room is full.");
...
test("submit racing owner delete never resurrects the room", async () => {
  ...
  if (stored === null) {
    ...
    assert.equal((await after.json()).error, "not_found");
    assert.equal(await store.get("sub-del"), null);
  }
});

test("storage unavailable returns a safe error without writing", async () => {
  const seeded = room("down", { members: [member("ada")] });
  releaseStore?.();
  releaseStore = await acquireStoreForTests({
    get: async () => structuredClone(seeded),
  ...
  mockCookieMap({ "club_owner_down": "owner", "club_member_down": "ada-token" });
  const deleted = await DELETE(new Request("http://localhost", { method: "DELETE" }), params("down"));
  const posted = await submitWork(jsonRequest("http://localhost/submit", { title: "X", body: "Y" }), params("down"));
  const joined = await joinRoom(jsonRequest("http://localhost/join", { name: "Ada" }), params("down"));
  ...
  assert.equal((await posted.json()).message.includes("unavailable"), true);
  assert.equal((await joined.json()).message.includes("unavailable"), true);
});

test("join 404 and delete forbidden keep the original payload shape", async () => {
  mockCookieMap({ "club_owner_real": "wrong" });
  const missing = await joinRoom(jsonRequest("http://localhost/join", { name: "Ada" }), params("absent"));
  assert.equal(missing.status, 404);
  assert.deepEqual(await missing.json(), { error: "not_found" });

  await store.createIfAbsent(room("real"));
  const denied = await DELETE(new Request("http://localhost", { method: "DELETE" }), params("real"));
